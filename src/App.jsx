...
function App() {
    const [percentages, setPercentages] = useState({
        ...
        rent: 0,
        buying_products: 0,
        debts: 0, // إضافة نسبة الديون
    });

    const [totalAmount, setTotalAmount] = useState("");
    const [showMessage, setShowMessage] = useState(false);
    const [errorMsg, setErrorMsg] = useState(""); // حالة جديدة لرسالة الخطأ
    ...
    // التحقق من المجموع وحفظ النسب
    const handleSaveToLocalStorage = () => {
        // حساب مجموع النسب الحالية
        const totalPercentage =
            (Number(percentages.profit) || 0) + (Number(percentages.zakat) || 0) + (Number(percentages.rent) || 0) + (Number(percentages.buying_products) || 0) + (Number(percentages.debts) || 0);

        // التحقق من المجموع
        if (totalPercentage > 100) {
            setErrorMsg(`مجموع النسب الحالية هو ${totalPercentage}% وهو أكبر من 100%.. يرجى تعديلها لتصبح 100% بالضبط 😅`);
            return; // إيقاف الدالة وعدم الحفظ
        } else if (totalPercentage < 100) {
            setErrorMsg(`مجموع النسب الحالية هو ${totalPercentage}% وهو أقل من 100%.. يرجى تعديلها لتصبح 100% بالضبط 😅`);
            return; // إيقاف الدالة وعدم الحفظ
        }

        // إذا كان المجموع 100% بالظبط، يتم الحفظ
        setErrorMsg("");
            ...
            <div id="container" className="max-w-5xl mx-auto px-4 mt-8">
                ...
                <div className="bg-white/70 backdrop-blur-md rounded-3xl shadow-xl p-8 border border-pink-200">
                    ...
                    <form
                        ...
                        <div className="flex flex-row flex-wrap justify-center gap-6 w-full">
                            ...
                            <div className="flex flex-col gap-2 w-32">
                                <label htmlFor="debts" className="font-semibold text-pink-700 text-lg text-center">
                                    الديون
                                </label>
                                <input
                                    className="border-2 border-pink-200 focus:border-pink-500 focus:ring-2 focus:ring-pink-200 rounded-xl p-2 text-center text-lg outline-none transition-all"
                                    type="number"
                                    value={percentages.debts}
                                    onChange={(e) => handlePercentageChange("debts", e.target.value)}
                                    id="debts"
                                />
                            </div>
                        ...
                        <button
                            type="submit"
                            className="mt-4 px-8 py-3 bg-gradient-to-r from-pink-400 to-purple-400 text-white hover:from-pink-500 hover:to-purple-500 hover:shadow-lg hover:shadow-pink-300/50 hover:-translate-y-1 transform transition-all duration-300 font-bold text-xl rounded-full">
                            حفظ النسب ✨
                        </button>
                    </form>
                </div>
                ...
                {errorMsg && (
                    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity">
                        <div className="bg-white border-2 border-red-300 p-8 rounded-3xl shadow-2xl flex flex-col items-center transform transition-transform scale-105 max-w-sm text-center">
                            <p className="text-4xl mb-3">⚠️</p>
                            <p className="text-xl font-bold text-red-500 mb-2">عذراً!</p>
                            <p className="text-gray-700 mb-6 font-medium leading-relaxed">{errorMsg}</p>
                            <button onClick={() => setErrorMsg("")} className="px-8 py-2 bg-red-400 text-white rounded-full font-bold hover:bg-red-500 transition-colors shadow-md hover:shadow-lg">
                                حسناً، سأعدلها
                            </button>
                        </div>
                    </div>
                ...
                {/* نافذة التأكيد للنجاح (Success Modal) */}
                {showMessage && (
                    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity">
                        <div className="bg-white border-2 border-pink-300 p-8 rounded-3xl shadow-2xl flex flex-col items-center transform transition-transform scale-105">
                            ...
                            <p className="text-gray-600 mb-6 text-center">تم حفظ النسب الخاصة بك بنجاح (المجموع 100%).</p>
                            <button
                                onClick={() => setShowMessage(false)}
                                className="px-8 py-2 bg-pink-500 text-white rounded-full font-bold hover:bg-pink-600 transition-colors shadow-md hover:shadow-lg">
                                إغلاق
                            </button>
                        </div>
                    </div>
                ...
                <div className="bg-white/70 backdrop-blur-md rounded-3xl shadow-xl p-8 border border-pink-200">
                    ...
                    <div id="outputs" className="flex flex-row flex-wrap justify-center gap-6 w-full mx-auto">
                        <div className="group flex flex-col items-center bg-gradient-to-b from-white to-pink-50 border border-pink-200 p-6 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 w-44 cursor-default">
                            ...
                            <span className="text-2xl font-black text-pink-500 mt-2">{calculateDistribution(percentages.profit)}</span>
                        </div>

                        <div className="group flex flex-col items-center bg-gradient-to-b from-white to-pink-50 border border-pink-200 p-6 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 w-44 cursor-default">
                            <div className="w-12 h-12 bg-pink-100 text-pink-500 rounded-full flex items-center justify-center text-2xl mb-3 group-hover:scale-110 transition-transform">🤲</div>
                            <h3 className="text-xl font-bold text-gray-700">الزكاة</h3>
                            <span className="text-2xl font-black text-pink-500 mt-2">{calculateDistribution(percentages.zakat)}</span>
                        </div>

                        <div className="group flex flex-col items-center bg-gradient-to-b from-white to-pink-50 border border-pink-200 p-6 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 w-44 cursor-default">
                            <div className="w-12 h-12 bg-pink-100 text-pink-500 rounded-full flex items-center justify-center text-2xl mb-3 group-hover:scale-110 transition-transform">🏠</div>
                            <h3 className="text-xl font-bold text-gray-700">الإيجار</h3>
                            <span className="text-2xl font-black text-pink-500 mt-2">{calculateDistribution(percentages.rent)}</span>
                        </div>

                        <div className="group flex flex-col items-center bg-gradient-to-b from-white to-pink-50 border border-pink-200 p-6 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 w-44 cursor-default">
                            <div className="w-12 h-12 bg-pink-100 text-pink-500 rounded-full flex items-center justify-center text-2xl mb-3 group-hover:scale-110 transition-transform">💳</div>
                            <h3 className="text-xl font-bold text-gray-700">الديون</h3>
                            <span className="text-2xl font-black text-pink-500 mt-2">{calculateDistribution(percentages.debts)}</span>
                        </div>
                        ...
                        <div className="group flex flex-col items-center bg-gradient-to-b from-white to-pink-50 border border-pink-200 p-6 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 w-44 cursor-default">
                            ...
                            <h3 className="text-xl font-bold text-gray-700 text-center">بضاعة</h3>
                            <span className="text-2xl font-black text-pink-500 mt-2">{calculateDistribution(percentages.buying_products)}</span>
                        </div>
                    </div>
                </div>
            </div>
